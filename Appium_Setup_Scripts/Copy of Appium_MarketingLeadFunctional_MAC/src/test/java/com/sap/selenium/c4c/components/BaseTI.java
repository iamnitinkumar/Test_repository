	...
	*/
	...
	public void clickButton(String button) throws Exception{
		...
		case"EDIT":
			...
			System.out.println(this.editButton);
	...
	protected boolean NaviagteToFacet(String facetPartialID, String measurementName) throws InterruptedException {
        WebElement scrollOpenfacet;
        WebElement facetElement;
        boolean facetAvailable=false;
        int i=0;
        ...
        {
              scrollOpenfacet = driver.findElement(By.className("sapUiUx3NavBarForward"));
              do
              {
                     try
                     {
                            
                            facetElement = this.elementToFindExplicit(By2.partialId(facetPartialID), 2);
                 if(facetElement.isDisplayed()==true){   
                      facetAvailable=true;                       
                      facetElement.click();                      
                     ...
                     {                          
                            i++;
                            scrollOpenfacet.click();
                            if(i>4){
                                   break;
                            }                    
                     }
              }
              while(facetAvailable == false);
        }
        else
        {

              try{
                     this.elementToClick(By2.partialId(facetPartialID), 10);                            
              }
              catch(Exception e){
                     Assert.fail("Facet with partialID " + facetPartialID + " not found in WebPage" + e.getLocalizedMessage());
              }
...
*/
	...
	protected WebElement getAction(String actionButtonPartialId,String actionPartialId) throws Exception {

...
public class AttachmentFacet extends BaseFacet {
	...
	protected void FromLibrary(String searchString, String valueHelpName,String table_PartialId) throws Exception {
		...
		if ((elem = this.getElementIfExisting(By2.startsWithAndPartialId(table_PartialId, "-table"), 5)) != null) {
			int tablerows = driver.findElements(By.xpath("//table[contains(./@id," + ConstantAttachment.Attachement_Table_PartialId  + ")]//tbody//tr")).size();
			...
			if (tablerows > 0 && tablerows == 2) 
				...
				if (existsElement("//table[contains(./@id, '" + table_PartialId + "') ]//tbody//tr[1]//td[1]//div//span") == true)
						...
						{
								elementToClick(By2.startsWithAndPartialId("embeddedcomponent",	"-close"), 5);
						...
						{
								elementToClick(By2.Xpath("//table[contains(./@id, '" + table_PartialId + "') ]//tbody//tr[1]"),5);

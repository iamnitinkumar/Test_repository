...
public class Base {
	 ...
	 */
	protected boolean elementToClick(By by, int waitTime) throws Exception {
	...
	protected boolean elementToClickLast(By by, int waitTime) throws Exception {
...
protected WebElement elementToFindExplicit(By by, int waitTime)
    {
	    ...
	    /*
	     * In cases of repeated usage of the same UI components, e.g. several appointment QC, the save buttons of the previous dialogs are 
	     * still lurking around and would be found by the plain findElement (findElements returns multiple entries)
	     * Therefore the element has to be taken directly from the wait.until ...
	     */
     ...
     * @param waitTime time to wait before the element is searched
     * @return element if it exists or null
     */
    protected WebElement getElementIfExisting(By by, int waitTime)
    {
    ...
    /**
     * Open value help, search for entry, select first hit list entry or cancel in case of no hits
     * @param valueHelpId
     * @param hitListItemId
     * @param searchString
     * @param valueHelpName
     ...
     */
    protected void valueHelpDialog(String valueHelpId, String searchString, String valueHelpName, String hitListEntryIDBegin) throws InterruptedException
    {
    	//Select account via value help, entering it directly doesn't always work
    	PerformanceRunner.startMeasurement("Open_"+valueHelpName+"_ValueHelp");
    	elementToFindExplicit(By2.startsWithAndPartialId(valueHelpId, "ButtonValueHelp"), 60).click();
		...
		Thread.sleep(4000);
		WebElement elem;
		elem = this.elementToFindExplicit(By2.startsWithAndPartialId("BasicFind", "-cb-input"), 5);
		
		
		PerformanceRunner.startMeasurement("Type_"+valueHelpName+"_ValueHelp");
		...
		if ((elem = this.getElementIfExisting(By2.startsWithAndPartialId(hitListEntryIDBegin, "-col0-row0"), 5)) != null)
	    {
	    	PerformanceRunner.startMeasurement("Select_"+valueHelpName+"_ValueHelp");
	    	elem.click();
	    	PerformanceRunner.stopMeasurement();
	    }
	    else
    ...
    /**
     * Enter passed value into dropdown menu
     * Menu is first selected, then cleared and finally, the value is put in followed by ENTER
     * @param elem dropdown menu input field
	...
	protected void fillDropDownFilter(By by, String value, String object) throws Exception
	{
	...
	protected void fillDropDown(WebElement elem, String value) throws InterruptedException
	{
		elem.click();
		Thread.sleep(1000);
		elem.clear();
	...
	//TODO: replica with changes
	    ...
	    protected void valueHelpDialog2(String valueHelpId, String searchString, String valueHelpName) throws InterruptedException
	    {
	    	elementToClear(By2.startsWithAndPartialId(valueHelpId, "-input"),60);
	    	PerformanceRunner.startMeasurement("Type_"+valueHelpName+"_ValueHelp");
	    	elementToSendKey(By2.startsWithAndPartialId(valueHelpId, "-input"),60,searchString);
	    	elementToSendEnter(By2.startsWithAndPartialId(valueHelpId, "-input"),60);
	    	PerformanceRunner.stopMeasurement();
	...
	//Changes made for Appium
	    public void fill_CompundField(String inputField_PartialID,String dropdown_PartialID, String value) 
		{
...

			{
				...
				try {
					...
					this.elementToFindExplicit(By2.partialId(inputField_PartialID),10).clear();
					Thread.sleep(5000);
					// fill the inputfield value
					this.elementToFindExplicit(By2.partialId(inputField_PartialID),10).sendKeys(values[0]);
					Thread.sleep(2000);
					
					// send keys for currency code
					
					//-------------Changes made for Appium-------------
					fillDropDownMobile(By2.startsWithAndPartialId(dropdown_PartialID, "-input"), values[1]); //change in appium
					//-------------------------------------------------
					
					//fillDropDown(By2.startsWithAndPartialId(dropdown_PartialID, "-input"), values[1]);
		...
		public void DatePicker(String labelName, String date) {
			...
			try {
				...
				driver.findElement(By2.elementbylabelwithElementTypeandPartialID(labelName, "div", "CALENDAR-icon")).click();
				Thread.sleep(1000);

				Select drop_month = new Select(driver.findElement(By.className("ui-datepicker-month")));
				drop_month.selectByVisibleText(dates[1]);

				Thread.sleep(2000);
				

				Select drop_year = new Select(driver.findElement(By.className("ui-datepicker-year")));
				...
				String monthValue = driver.findElement(By.xpath("//option[@selected='selected']")).getAttribute("value");
				driver.findElement(By.xpath("//a[text()='" + dates[0] + "'][../@data-month='"+ monthValue + "']")).click();
		...
		public void DatePickerbyID(String partialID, String date) {
			...
			try {
				...
				//driver.findElement(By2.elementbylabelwithElementTypeandPartialID(labelName, "div", "CALENDAR-icon")).click();
				driver.findElement(By2.startsWithAndPartialId(partialID,"CALENDAR-icon")).click();
				Thread.sleep(1000);
			...
			if (value.contains(" ")) {
				...
				try {
					if (values[0] != null) {
						...
						ele = this.elementToFindExplicit(By2
								.startsWithAndPartialId(partialID, "CALENDAR-input"), 1);
						if (ele.isEnabled() && ele != null) {
...
//				partialID = "CALENDAR-input";
				ele = this.elementToFindExplicit(By2.startsWithAndPartialId(partialID, "CALENDAR-input"), 1);
				if (ele.isEnabled() && ele != null) {
		...
		public boolean isExceptionDialogExist()
		{
			try {
				...
				WebElement ExceptionElement = driver.findElement(By2.classNameByPartialId("sapUiDlgModal"));
		...
		public boolean isNotificationMessageAreaExist()
		{
			try 
			{
				WebElement notificationelement = driver.findElement(By2.classNameByPartialId("sapUiNotificationBar"));
		...
		public String getNotificationMessage() throws Exception
		{
			...
			if (notificationelement != null)
			{
				System.out.println("isNotifierContainerPresent() : notificationelement not null");

				notificationelement.getAttribute("id");
				//TODO : to add a element click step once we are able to get all the messages in the area. For loop is placeholder for the same

				List<WebElement> elements = driver.findElements(By2.classNameByPartialId("sapUiNotifierMessageText"));
			...
			{
				System.out.println("isNotifierContainerPresent() : notificationelement  null");
		...
		public void TakeScreenShot() throws IOException 
		{
			...
			File file = new File("C:\\Selenium_ScreenShot");  //write to my directory
			if (!file.exists()) {
				if (file.mkdir()) {
				...
				FileUtils.copyFile(scrFile, new File(file + "\\screenshot_" + this.testName
		...
		/**
	       ...
	       */
	       protected void valueHelpDialog_New(String valueHelpId, String searchString,
	              ...
	              if ((elem = this.getElementIfExisting(
	                           By2.startsWithAndPartialId(tableParialID, "-table"), 5)) != null) {
	                     ...
	                     if (tablerows > 0 && tablerows == 2) {
	                    	 ...
	                    	 String elemPath = "//table[contains(./@id, '" + tableParialID + "') ]//tbody//tr[1]//td[1]//div//span";
	                           ...
	                           this.elementToFindExplicit(
	                                         By2.Xpath("//table[contains(./@id, '"
	                                                      + tableParialID + "') ]//tbody//tr[1]"),
	                                         5).click();
	     ...
	     /**
	   	...
	   	/**
	   	 * @author rahul.agarwal01@sap.com
	   	 * Method Name: WebElementListSize
	   	 * @param by
